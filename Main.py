...
if version== '1pixel':
    ...
    # pathtoui=resource_path(r'GUIfiles\gui.ui')
    # Ui_MainWindow, QMainWindow = loadUiType(r'GUIfiles\gui.ui')
# elif version== '6pixels':
#     sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)),'GUIfiles'))
#     Ui_MainWindow, QMainWindow = loadUiType(r'C:\Users\serjw\Documents\nBox\PythonScripts\SolarSim\SolarSimPklab-SERIS-210107exe\labCode\GUIfiles\gui_6pixels.ui')
...
usbflashdriveaddress='200528448113C0E2A1EC'
hostnamecomputer = 'NanoLab'
# hostnamecomputer = 'SERIS-WERNER'
IPHostaddress='192.168.68.104'
MACaddress='cc:f9:e4:d5:93:05'
expiracydate = datetime.datetime.strptime('31/12/2022 6:00', "%d/%m/%Y %H:%M")
...
- Rep: the graphs should be changable during testing. user should be able to switch from Eff to Voc to ... without interferring with measurement. 

- error if Keithley is not configured correctly: remind the user with proper configurations of keithley 2400
can we programmatically force the keithley to reconfigure at start?

- check Pmax scan condition of NREL (poster)

- turn off keithley after measurement, empty queue/memory, put voltage to 0. queue overflow
-363 error, system overflow
baud rate could be changed to higher value: 115200

- artefacts in JV scans. little bumps at regular positions. not same in forward and reverse.
what are they? software, hardware ? comm? 


- check the ASTM and IEC standards on how to measure/calculate the JV parameters



- new degradation stability testing protocol with stepwise stabilized IV curve
    ...
    stabilisation time: leave max 1min, if not met criteria within time, indicate that in report and move on to next (e.g. red data points on IV curve)
...
class Main(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        # self.ui.textEdit_Keithley.append(QtCore.QDateTime.currentDateTime().toString()+': '+'')
        # print('moving on to DB:')
        self.connecttoDB()
        
        
        self.settings = QSettings('SERIS', 'PyPkLab')
        ...
        # self.fig5 = Figure()
        # self.StabGraph1 = self.fig5.add_subplot(221) #initial and final JV scans
        # self.StabGraph1.set_xlabel('Voltage (mV)')
        # self.StabGraph1.set_ylabel('Current density (mA/cm'+'\xb2'+')')
        # self.StabGraph2 = self.fig5.add_subplot(222) #step-wise JV vs time
        # self.StabGraph2.set_xlabel('Time (s)')
        # self.StabGraph2.set_ylabel('Power density (mW/cm'+'\xb2'+')')
        # self.StabGraph2bis = self.StabGraph2.twinx()
        # self.StabGraph2bis.set_ylabel('Voltage (mV)')
        # self.StabGraph3 = self.fig5.add_subplot(223) #power curve with stabilized data from C&E
        # self.StabGraph3.set_xlabel('Voltage (mV)')
        # self.StabGraph3.set_ylabel('Power density (mW/cm'+'\xb2'+')')
        # self.StabGraph4 = self.fig5.add_subplot(224) #tracking data
        # self.StabGraph4.set_xlabel('Time (s)')
        # self.StabGraph4.set_ylabel('Power density (mW/cm'+'\xb2'+')')
        # self.StabGraph4bis = self.StabGraph4.twinx()
        # self.StabGraph4bis.set_ylabel('Voltage (mV)')
        # self.addmpl(self.fig5,self.ui.gridLayout_21,self.ui.MPLwidgetStabPowerCurve)
        ...
        # self.ui.actionSave_settings_gen.triggered.connect(lambda: SaveParamTemplateGeneric(self,self.ui, self.settings))

        # self.ui.pushButton_conntest.clicked.connect(lambda: connectToKeithley(keithleyAddress))
...
#%%#############
    ...
    def SaveSession(self):
        ...
        # need to change for defined folder, like DB
...
#%%######################################################################################################
    ...
    #        self.shutter('CloseShutter',keithleyObject)
            # setRearTerminal(keithleyObject)
            # shutdownKeithley(keithleyObject)
            RefDiodeChecked=1
    ...
    def launchSequence(self, keithleyObject, pixels, pixcolorslist, scandirections, Rep):
        ...
        # elif self.sequence.split('-')[0]=='StepwiseJV':
        #     self.ui.label_ongoingmeasurement.setText('The measurement sequence has started... StepwiseJV')
        #     self.ui.Figure_tabWidget.setCurrentIndex(3)
        #     self.sequence=self.sequence[11:]
        #     self.shutter('OpenShutter',keithleyObject)
            # QtTest.QTest.qWait(int(self.ui.doubleSpinBox_JVdelayshutter.value()))
            # self.PlotStepwiseJVThreaded(keithleyObject, pixels, pixcolorslist, scandirections, Rep)
            
        # elif self.sequence.split('-')[0]=='StabProtocol':
        #     self.ui.label_ongoingmeasurement.setText('The measurement sequence has started... StabProtocol')
        #     self.ui.Figure_tabWidget.setCurrentIndex(3)
        #     self.sequence=self.sequence[13:]
        #     self.shutter('OpenShutter',keithleyObject)
        #     QtTest.QTest.qWait(int(self.ui.doubleSpinBox_JVdelayshutter.value()))
        #     self.StabProtocolThreaded(keithleyObject, pixels, pixcolorslist, scandirections, Rep)
...
class Window(QtWidgets.QDialog):
    ...
    def MainReal(self): 
        ...
        # keithleyAddress='ASRL1::INSTR'
        # keithleyAddress='ASRL5::INSTR'
        # filename = 'KeithleyAddresses.txt' 
        # if '_MEIPASS2' in os.environ:
        #     filename = os.path.join(os.environ['_MEIPASS2'],'KeithleyCode', filename)
        # else:
        # filename = os.path.join('KeithleyCode', filename)
        # filetoread = open(filename, 'r', encoding='ISO-8859-1')
        # filetoread = open(r'KeithleyCode\KeithleyAddresses.txt',"r", encoding='ISO-8859-1')
        # keithleyAddress = filetoread.read().splitlines()
        ...
        # filename = 'USBflashdrive.txt' 
        # if '_MEIPASS2' in os.environ:
        #     filename = os.path.join(os.environ['_MEIPASS2'],'Otherfunctions', filename)
        # else:
        # filename = os.path.join('Otherfunctions', filename)
        # filetoread = open(filename, 'r', encoding='ISO-8859-1')
        # filetoread = open(r'Otherfunctions\USBflashdrive.txt',"r", encoding='ISO-8859-1')
        # usbflashdriveaddress = filetoread.read().splitlines()[0]
        # print('usb address is ',usbflashdriveaddress)
        print('')
        print(Kconnresult)
        print('')
    ...
    def checkUSBFlashDrive(self):
        global usbflashdriveaddress, IPHostaddress, MACaddress, expiracydate, hostnamecomputer
        # print('usb address is ',usbflashdriveaddress)
...
#        sys.exit()
        ...
        # if gma() == MACaddress:
        #     self.ui.textEdit_Keithley.append(QtCore.QDateTime.currentDateTime().toString()+': '+'Valid MAC address')
        # else:
        #     QMessageBox.information(self,'Unauthorized use - Fraud alert', 'It seems that you are not operating on an authorized computer.')
        #     app.quit()
        #     window.close()
        #     sys.exit()
        if socket.gethostname() == hostnamecomputer:
            print(QtCore.QDateTime.currentDateTime().toString()+': '+'Valid computer')
            # self.ui.textEdit_Keithley.append(QtCore.QDateTime.currentDateTime().toString()+': '+'Valid computer')
        # else:
        #     QMessageBox.information(self,'Unauthorized use - Fraud alert', 'It seems that you are not operating on an authorized computer.')
            # app.quit()
            # window.close()
        ...
        if datetime.datetime.now() < expiracydate:
            print('Remaining validity time: '+str(expiracydate-datetime.datetime.now()))
